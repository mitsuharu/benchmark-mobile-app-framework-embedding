...
final class SearchResultsStore: ObservableObject, RepoSearchBridgeDelegate {
  ...
  /// The keyword handed to the Flutter screen when it is opened.
...
/// Flutter screen, and the results come back through `RepoSearchBridge`.
struct ContentView: View {
  ...
  var body: some View {
      ...
      List {
        ...
        Section("埋め込み画面") {
          ...
          Button {
            ...
            Label("Flutter の画面を開く", systemImage: "magnifyingglass")
          }
    ...
    // List disappear, and the Flutter screen reports its results while it is on top.
...
private struct RepoSearchScreen: View {
  ...
  var body: some View {
    FlutterScreen(keyword: keyword) { dismiss() }
      .ignoresSafeArea(edges: .bottom)
      .navigationTitle("Repo Search (Flutter)")
...
/// Shows the cached engine's Dart app in a `FlutterViewController`.
private struct FlutterScreen: UIViewControllerRepresentable {
  let keyword: String
  let onClose: () -> Void
