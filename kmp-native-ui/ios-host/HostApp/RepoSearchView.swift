...
///
/// Written in SwiftUI, like the native baseline's screen; only the state and
/// the network call come from the shared Kotlin module.
struct RepoSearchView: View {
  @Environment(\.dismiss) private var dismiss
  @StateObject private var model: RepoSearchViewModel

  init(keyword: String, apiBaseURL: URL) {
    _model = StateObject(
      wrappedValue: RepoSearchViewModel(keyword: keyword, apiBaseURL: apiBaseURL)
    )
  }
